...
use crate::types::*;
use enforcement::{AuthConn, RecordId, TypedRecordId, Principal, DBConn, DBCollection};
...
impl<'a, 'r> request::FromRequest<'a, 'r> for SessionConn {
    ...
    fn from_request(request: &'a request::Request<'r>) -> request::Outcome<SessionConn, !> {
        Outcome::Success(request.cookies()
            .get_private("user_id")
            .and_then(|cookie| from_str(cookie.value()).ok())
            .map(|id: RecordId| {
              SessionConn(DB_CONN.clone().as_princ(Principal::Id(id)))
            })
            .unwrap_or_else(|| {
    ...
    fn from_request(request: &'a request::Request<'r>) -> request::Outcome<AdminConn, !> {
      UserId::from_request(request).and_then(|uid| {
        ...
        User::find_by_id(&conn, uid.0)
             .and_then(|u| u.admin)
             .and_then(|is_admin| is_admin.then(|| AdminConn(DB_CONN.clone().as_princ(Principal::Static("Admin")))))
             .or_forward(())
      })
    }
}
